...
def pulse_time_efficiency(off_resonances: np.ndarray, magnetisation: np.ndarray,
    ...
    if not is_inversion:
        ...
        plt.text(np.median(off_resonances), (final[len(first) // 2] + len(first) / 40) * delta_time * 1e3,
        ...
        plt.plot(off_resonances, delta_time * 1e3 * final, color='white', ls='--')
        plt.text(np.median(off_resonances), (final[len(first) // 2] + len(final) / 40) * delta_time * 1e3,
                 '$M_{z}$ < -0.95', color='white', horizontalalignment='center')

        plt.title('$M_{z}$')

    plt.subplot(1, 2, 2)
    plt.plot(off_resonances, delta_time * 1e3 * (final - first))
    plt.grid()
